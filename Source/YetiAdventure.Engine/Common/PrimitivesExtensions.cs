...
namespace YetiAdventure.Engine.Common
{
    public static class PrimitivesExtensions
    {
        ...
        public static void DrawPolygon(this SpriteBatch inSpritebatch, Vector2[] inVertices, Color inColor, bool inbClosed = true)
        {
            // Step through pairs of vertices to draw a polygon in line segments.
            for (int segmentIndex = 1; segmentIndex < inVertices.Length; ++segmentIndex)
            {
                Vector2 prev = inVertices[segmentIndex - 1];
                Vector2 current = inVertices[segmentIndex];

                DrawLine(inSpritebatch, prev, current, inColor);
            }

            if (inbClosed)
            {
                // Close the polygon by drawing a line from the first to the last vertex.
                DrawLine(inSpritebatch, inVertices[0], inVertices[inVertices.Length - 1], inColor);
            }
        }
