...
namespace FarseerPhysics.Dynamics
{
    ...
    public class World
    {
        ...
        private DebugViewBase mDebugDrawer = null;
        ...
        /// </summary>
        /// <param name="inDebugDrawer">A debug drawer instance.</param>
        public void SetDebugDrawer(DebugViewBase inDebugDrawer)
        {
            mDebugDrawer = inDebugDrawer;

            //ContactManager.PreSolve = mDebugDrawer.OnPreSolve;
        }

        public void DrawDebugData()
        {
            if ((mDebugDrawer.Flags & DebugViewFlags.Shape) == DebugViewFlags.Shape)
            {
                foreach (Body b in BodyList)
                {
                    ...
                    {
                        if (b.Enabled == false)
                            DrawShape(f, xf, mDebugDrawer.InactiveShapeColor);
                        else if (b.BodyType == BodyType.Static)
                            DrawShape(f, xf, mDebugDrawer.StaticShapeColor);
                        else if (b.BodyType == BodyType.Kinematic)
                            DrawShape(f, xf, mDebugDrawer.KinematicShapeColor);
                        else if (b.Awake == false)
                            DrawShape(f, xf, mDebugDrawer.SleepingShapeColor);
                        else
                            DrawShape(f, xf, mDebugDrawer.DefaultShapeColor);
                    }
                }
            }

/*
            if ((mDebugDrawer.Flags & DebugViewFlags.ContactPoints) == DebugViewFlags.ContactPoints)
            {
                ...
                for (int i = 0; i < _pointCount; ++i)
                {
                    ...
                    if (point.State == PointState.Add)
                        mDebugDrawer.DrawPoint(point.Position, 0.1f, new Color(0.3f, 0.95f, 0.3f));
                    else if (point.State == PointState.Persist)
                        mDebugDrawer.DrawPoint(point.Position, 0.1f, new Color(0.3f, 0.3f, 0.95f));

                    if ((Flags & DebugViewFlags.ContactNormals) == DebugViewFlags.ContactNormals)
                    {
                        Vector2 p1 = point.Position;
                        Vector2 p2 = p1 + axisScale * point.Normal;
                        mDebugDrawer.DrawSegment(p1, p2, new Color(0.4f, 0.9f, 0.4f));
                    }
                }
            ...
            if ((mDebugDrawer.Flags & DebugViewFlags.PolygonPoints) == DebugViewFlags.PolygonPoints)
            {
                    ...
                    foreach (Fixture f in body.FixtureList)
                    {
                        ...
                        if (polygon != null)
                        {
                            ...
                            for (int i = 0; i < polygon.Vertices.Count; i++)
                            {
                                ...
                                mDebugDrawer.DrawPoint(tmp, 0.1f, Color.Red);
                            }
                        }
                    }
                }
            ...
            if ((mDebugDrawer.Flags & DebugViewFlags.Joint) == DebugViewFlags.Joint)
            {
            ...
            if ((mDebugDrawer.Flags & DebugViewFlags.AABB) == DebugViewFlags.AABB)
            {
                Color color = new Color(0.9f, 0.3f, 0.9f);
                IBroadPhase bp = ContactManager.BroadPhase;
            ...
            if ((mDebugDrawer.Flags & DebugViewFlags.CenterOfMass) == DebugViewFlags.CenterOfMass)
            {
                foreach (Body b in BodyList)
                {
                    Transform xf;
                    b.GetTransform(out xf);
                    xf.p = b.WorldCenter;
                    mDebugDrawer.DrawTransform(ref xf);
                }
            }

            if ((mDebugDrawer.Flags & DebugViewFlags.Controllers) == DebugViewFlags.Controllers)
            {
            ...
            /*
            if ((mDebugDrawer.Flags & DebugViewFlags.DebugPanel) == DebugViewFlags.DebugPanel)
                DrawDebugPanel();
            */
        }
        ...
        private void DrawJoint(Joint joint)
        {
            ...
            switch (joint.JointType)
            {
                case JointType.Distance:
                    mDebugDrawer.DrawSegment(p1, p2, color);
                    break;
                case JointType.Pulley:
                    ...
                    mDebugDrawer.DrawSegment(p1, p2, color);
                    mDebugDrawer.DrawSegment(p1, s1, color);
                    mDebugDrawer.DrawSegment(p2, s2, color);
                    break;
                case JointType.FixedMouse:
                    mDebugDrawer.DrawPoint(p1, 0.5f, new Color(0.0f, 1.0f, 0.0f));
                    mDebugDrawer.DrawSegment(p1, p2, new Color(0.8f, 0.8f, 0.8f));
                    break;
                case JointType.Revolute:
                    mDebugDrawer.DrawSegment(x1, p1, color);
                    mDebugDrawer.DrawSegment(p1, p2, color);
                    mDebugDrawer.DrawSegment(x2, p2, color);

                    mDebugDrawer.DrawSolidCircle(p2, 0.1f, Vector2.Zero, Color.Red);
                    mDebugDrawer.DrawSolidCircle(p1, 0.1f, Vector2.Zero, Color.Blue);
                    break;
                ...
                case JointType.FixedRevolute:
                    mDebugDrawer.DrawSegment(x1, p1, color);
                    mDebugDrawer.DrawSolidCircle(p1, 0.1f, Vector2.Zero, Color.Pink);
                    break;
                ...
                case JointType.FixedPrismatic:
                    ...
                    break;
                case JointType.Gear:
                    mDebugDrawer.DrawSegment(x1, x2, color);
                    break;
                default:
                    mDebugDrawer.DrawSegment(x1, p1, color);
                    mDebugDrawer.DrawSegment(p1, p2, color);
                ...
                case ShapeType.Circle:
                    {
                        ...
                        mDebugDrawer.DrawSolidCircle(center, radius, axis, color);
                    }
                    break;
                ...
                case ShapeType.Polygon:
                    {
                        ...
                        {
                            tempVertices.Add(MathUtils.Mul(ref xf, poly.Vertices[i]));
                        }

                        mDebugDrawer.DrawSolidPolygon(tempVertices.ToArray(), vertexCount, color);
                    }
                    break;
                ...
                case ShapeType.Edge:
                    {
                        ...
                        mDebugDrawer.DrawSegment(v1, v2, color);
                    }
                    break;
        ...
        private void DrawAABB(ref AABB aabb, Color color)
        {
            ...
            mDebugDrawer.DrawPolygon(verts, 4, color);
        }
